...
def test_the_ci_approval_targets_the_pull_request_invocation() -> None:
    # The approval has to read the invocation that actually opens the PR. After
    # ENG-13319 split the action in two, `steps.release` carries
    # `skip-github-pull-request: true` — its `prs_created` is never 'true' and
    # its `pr` output is never set. A step left reading that id would silently
    # never fire, and the release PR would sit with its required checks unstarted:
    # the same dead end ENG-13320 exists to close, reached a different way.
    rp = RELEASE_PLEASE_PATH.read_text()
    assert "steps.release_pr.outputs.prs_created == 'true'" in rp
    assert "steps.release_pr.outputs.pr" in rp
    assert "steps.release.outputs.prs_created" not in rp, (
        "the invocation that skips the pull request never reports one as created"
    )
